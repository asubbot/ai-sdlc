...
- **Advisory pattern catalog (non-normative):** During stage 6, agents consult the advisory architecture-pattern index under `reference/architecture-patterns/` (see [06-system-design.skill.md](skills/06-system-design.skill.md), workflow step 3). The catalog does not change artefact paths, gates, or validate rules; if it is absent from the checkout, skills define the fallback.
